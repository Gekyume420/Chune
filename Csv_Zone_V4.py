...
def fetch_data_and_write_to_csv(computer_id1, computer_id2):
    ...
    # Function to process tasks and add 'id' to each task
    def process_tasks(data, id_suffix=''):
        tasks = []
        if data:
    ...
    tasks1 = process_tasks(data1)
    tasks2 = process_tasks(data2, '2')  # Suffix for id in the second dataframe

    df1 = pd.DataFrame(tasks1)
    df2 = pd.DataFrame(tasks2)

    # Calculate total points for each DataFrame, considering only non-zero values
    ...
    # Add a new row for 'Total Points'
